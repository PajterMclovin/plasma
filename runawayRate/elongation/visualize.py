...
Visualizes DREAM output data for a range of maximum elongations.
"""
...
# visualize all outputs
    ...
    for fp in os.listdir(outputDir):
        if fp.endswith(".h5"):
            ...
            maxElongation = fp[len('output'):-3]
            ax = plotRunawayRate(do, ax=ax, label=maxElongation, normalize=True)

else:
    raise ValueError('Expected at most one argument.')

# plot settings
plt.legend(title='max elongation')
