...
Visualizes DREAM output data for a range of triangularities.
"""
...
# visualize all outputs
    ...
    for fp in sorted(os.listdir(outputDir), key=lambda x: float(x[len('output'):-3])):
        ...
        if fp.startswith('output') and fp.endswith('.h5'):
            delta = float(fp[len('output'):-len('.h5')])    # extract triangularity
...
# plot settings
ax1.legend(title=r'Triangularity $\delta$')
ax1.set_ylabel(r'runaway rate $\gamma$ [s$^{-1}$ m$^{-3}$]')
ax2.set_ylabel('Effective passing fraction')
ax1.set_xlabel(r'minor radius $r$ [m]')
ax2.set_ylabel(r'height $z$[m]')
ax2.set_xlabel(r'minor radius $r$ [m]')
