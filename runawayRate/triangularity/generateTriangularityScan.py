...
def configureGrids(ds, maxTriangularity=None, verbose=False, visualize=False):
    ...
    float maxTriangularity :    Maximum triangularity (geometry parameter). (MAX MIN??)
    ...
    if maxTriangularity is None:
        maxTriangularity = p.MAX_TRIANGULARITY
    elif maxTriangularity < 0:
        pass

    delta = np.linspace(0, maxTriangularity, p.N_TRIANGULARITY)
...
if __name__ == "__main__":
    ...
    for maxTriangularity in scanValues:

    	# Generate DREAM setting with torodial geometry with varying maximum elongation
        ds = DREAMSettings()
        configureGrids(ds, maxTriangularity=maxTriangularity, visualize=(len(sys.argv)==2))
        configureEquations(ds)
        ds.output.setFilename(f'outputs/output{maxTriangularity}.h5')
        ds.other.include('fluid/runawayRate')
        ds.save(f'dream_settings/settings{maxTriangularity}.h5')
